...
int convert(std::istream & is, char (&block)[CSCConstants::NUM_DCFEBS][RAMPAGE_SIZE], unsigned int layerset = 1, char sep = '|') {
...
//
    ...
    int bytes=0;
    for (unsigned int i= 0; i < triads.size()/DCFEBs; i++){
        for (int j=0; j < DCFEBs; j++){
    	 	//std::cout << (std::bitset<8>)triads[i*DCFEBs+j] <<"\t";
            c = triads[i*DCFEBs+j];
		    //(*(oss[j])) << c;
            //std::cout<<" dcfeb "<< j<<" BX "<< (i)/6 <<" layer "<< (i)%6 <<" bytes "<< bytes << " c "<< (std::bitset<8>) c << std::endl;
            block[j][bytes] = c;
		    if ((i+1) %6 == 0){ 
                block[j][bytes+1] = char(0);
                block[j][bytes+2] = char(0);
            }
                //(*(oss[j])) << char(0) << char(0);
	    }
        bytes++;
    ...
    /*for (unsigned int i= 0; i < bytes; i++){
        std::cout <<" byte i "<< i <<" BX "<< (i)/6 <<" layer "<< (i)%6;  
        for (int j=0; j < DCFEBs; j++){
            std::cout <<" dcfeb "<< j <<" triads "<<(std::bitset<8>)  block[j][i]; 
        }
        std::cout << std::endl;
    }*/
    for(int i=0; i<DCFEBs; ++i) {
        //std::cout << "bits for DCFEB_n " <<i << " " <<  c_count[i]*8 + (c_count[i]/6)*16 << std::endl;
        while ( bytes < RAMPAGE_SIZE ) {
		//(*(oss[i])) << char(0) ;
        block[i][bytes]=char(0);
		bytes++;
...
/*int main(int argc, char * argv[]) {
    ...
    int layerset = 1;
    if (argc == 3) {
       std::istringstream(argv[2]) >> layerset;
       std::cout <<" take layer order case " << layerset << std::endl;
