...
FT_STATUS read_byte(uint8 slaveAddress, uint8 registerAddress, uint8 *data)
{
	...
	bytesTransfered=0;
	buffer[bytesToTransfer++]=registerAddress; /* Byte addressed inside EEPROM */
	status = p_I2C_DeviceWrite(ftHandle, slaveAddress, bytesToTransfer, buffer,&bytesTransfered, I2C_TRANSFER_OPTIONS_FAST_TRANSFER_BYTES|I2C_TRANSFER_OPTIONS_START_BIT);
	bytesToTransfer=2;
	bytesTransfered=0;
	status |= p_I2C_DeviceRead(ftHandle, slaveAddress, bytesToTransfer, buffer,&bytesTransfered, I2C_TRANSFER_OPTIONS_STOP_BIT |I2C_TRANSFER_OPTIONS_START_BIT|I2C_TRANSFER_OPTIONS_NACK_LAST_BYTE);
        printf(" bytesTransfered %d \n",bytesTransfered);
	*data = buffer[0];
        for(i=0;i<8;i++)printf("buffer  %d  %02x\n",i,buffer[i]);
	return status;
}
	...
	if(channels>0)
	{
		...
		/*	for(address=START_ADDRESS_EEPROM;address<END_ADDRESS_EEPROM;address++)
		{
			...
			printf("reading address %d data read=%d\n", address, data);
                ...
                status = read_byte(0x51,0x00, &data);
                if(FT_OK != status)printf(" read_byte: status error\n");
