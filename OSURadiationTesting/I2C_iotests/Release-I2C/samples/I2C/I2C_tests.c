	...
	if(channels>0)
	  {
		...
		if(devList.ID == 0x10c1)port_a_chan = i;
		if(devList.ID == 0x10c2)port_b_chan = i;
	      }


	    /* Open the first available channel */
	    status = I2C_OpenChannel(port_a_chan,&ftHandle0);
	    APP_CHECK_STATUS(status);
	    printf("\nhandle0=0x%x status=%d\n",ftHandle0,status);
            fprintf(Dataout,"\nhandle0=0x%x status=%d\n",ftHandle0,status);

	    /* Open the next available channel */
	    status = I2C_OpenChannel(port_b_chan,&ftHandle1);
	    APP_CHECK_STATUS(status);
